...
void FrameBuffer::Initialize(int32_t bufferWidth, int32_t bufferHeight, bool bIsEnableHDR)
{
	...
	GL_FAILED(glTexImage2D(GL_TEXTURE_2D, 0, bIsEnableHDR_ ? GL_RGBA16F : GL_RGBA, bufferWidth, bufferHeight, 0, GL_RGBA, bIsEnableHDR_ ? GL_FLOAT : GL_UNSIGNED_BYTE, nullptr));
