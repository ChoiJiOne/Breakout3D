...
void StaticMesh::Initialize(const std::vector<Vertex>& vertices, const std::vector<uint32_t>& indices)
{
	...
	GL_ASSERT(glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, Vertex::GetStride(), (void*)(offsetof(Vertex, tangent))), "failed to define an array of generic vertex attribute data");
	GL_ASSERT(glEnableVertexAttribArray(2), "failed to enable a generic vertex attribute array");

	GL_ASSERT(glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, Vertex::GetStride(), (void*)(offsetof(Vertex, bitangent))), "failed to define an array of generic vertex attribute data");
	GL_ASSERT(glEnableVertexAttribArray(3), "failed to enable a generic vertex attribute array");

	GL_ASSERT(glVertexAttribPointer(4, 2, GL_FLOAT, GL_FALSE, Vertex::GetStride(), (void*)(offsetof(Vertex, uv))), "failed to define an array of generic vertex attribute data");
